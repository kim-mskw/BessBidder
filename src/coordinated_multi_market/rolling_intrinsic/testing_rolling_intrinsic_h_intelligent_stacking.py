...
def derive_day_ahead_trades_from_drl_output(
    ...
    # round everything to to NAchkommastellen
    mask = df.capacity_trade != 0
    df = df[mask]
    # if df.capacity_trade.sum() != 0:
    #    return pd.DataFrame()
...
def simulate_period_stacked_hourly_products(
    da_bids_path: str,
    output_path: str,
    start_day,
    end_day,
    ...
    log_message = (
        "Running Rolling intrinsic H with the following parameters:\n"
        "Start Day: {start_day}\n"
        "End Day: {end_day}\n"
        "Threshold: {threshold}\n"
    ...
    while current_day < end_day:
        ...
        # Add the first row
        # all_trades.loc[2] = [pd.to_datetime("2021-12-31 16:00:00+01:00"), "buy", 1.0, -200,
        #                      pd.to_datetime("2022-01-01 10:00:00+01:00"), -200]
        # Add the second row
        # all_trades.loc[3] = [pd.to_datetime("2021-12-31 16:00:00+01:00"), "sell", 1.0, 500,
        #                      pd.to_datetime("2022-01-01 19:00:00+01:00"), 500]
        ...
        current_cycles += net_trades["net_buy"].sum()
...
def simulate_days_stacked_hourly_products(
    ...
    while current_day < list_dates[-1]:
        current_index = list_dates.index(current_day)
        current_day = current_day.replace(hour=0, minute=0, second=0, microsecond=0)
        ...
        # calculate number of days until end_day
        days_left = round((len(list_dates) - current_index) / 24)
        ...
        # set current day to current_day plus 1 day
        current_day = list_dates[current_index + 23]
