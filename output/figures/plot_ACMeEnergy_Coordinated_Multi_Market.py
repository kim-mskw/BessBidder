"""
Coordinated multi-market profit and State-of-Charge (SoC) plotting script for ACM e-Energy paper.
...
Requires:
- DA and IDC result CSVs for the coordinated strategy
...
if __name__ == "__main__":
    ...
    path_da = Path(
        "output/ACM_results/4.3_Final_Revision_Hail_Mary_131/basic_battery_dam_test_log_v3.csv"
    )
    path_idc = Path(
        "output/ACM_results/4.3_Final_Revision_Hail_Mary_131/rolling_intrinsic_intelligently_stacked_on_day_ahead_qh/bs15cr1rto0.86mc365mt10"
    )

    # Day-Ahead profit: aggregate daily
    df_da = load_data(path_da)  # using load data function to access RL outputs
    df_da.index = pd.to_datetime(df_da.index, utc=True).tz_convert("Europe/Berlin")
    df_da = df_da.drop_duplicates(keep="last")
    da_daily = df_da.groupby(df_da.index.date)["profit"].sum()

    # Intraday Continuous profit: aggregate daily
    df_idc = pd.read_csv(path_idc / "profit.csv", parse_dates=["day"])
    ...
    profits = profits[1:]  # remove the first entry (optional outlier removal)
    ...
    for i, date in enumerate(profits.index):
        path_trades = path_idc / f"trades/trades_{date}.csv"
        ...
        path_vwaps = path_idc / f"vwap/vwaps_{date}.csv"
        ...
        soc_da = pd.DataFrame(df_da["obs: soc_t"][df_da.index.date == date] * 100)
    ...
    plt.tight_layout()
    plt.savefig("output/figures/profit_plot_Coordinated_Multi_Market.png")
    plt.savefig("output/figures/profit_plot_Coordinated_Multi_Market.pdf", format="pdf")
    plt.show()
